 ...
 * @version 1.6.2
 */
abstract class Slice_Base_Plugin_Updater {

	private $api_url  = 'http://slicedinvoices.com/';
	private $api_data = array();
	private $name     = '';
	private $slug     = '';
	private $required = array( 'option_slug', 'plugin_file', 'item_name', 'version', 'author' );
	 ...
	 */
	public function __construct( $api_data = null ) {

		foreach ( $this->required as $property ) {
			if ( ! $this->{$property} ) {
				throw new Exception( $property . ' is required to be set by the extended class.' );
			}
		}

		$this->key_name    = $this->key_name ? $this->key_name : $this->option_slug . '_license_key';
		$this->status_name = $this->status_name ? $this->status_name : $this->option_slug . '_license_status';

		// retrieve our license keys from the DB
		$licenses = get_option( 'sliced_licenses' );

		$this->license_key    = isset( $licenses[ $this->key_name ] ) ? trim( $licenses[ $this->key_name ] ) : '';
		$this->license_status = isset( $licenses[ $this->status_name ] ) ? trim( $licenses[ $this->status_name ] ) : '';
	...
	public function init() {
		...
		if ( isset( $_POST[ $this->option_slug . '_license_activate'], $_POST[ $this->key_name ] ) ) {
			add_action( 'admin_init', array( $this, 'activate_license' ) );
		}

		// listen for our activate button to be clicked
		if ( isset( $_POST[ $this->option_slug . '_license_deactivate'] ) ) {
	...
	public function activate_license() {
		...
		if ( ! check_admin_referer( 'sliced_license_nonce', 'sliced_license_nonce_' . $this->option_slug ) ) {
	...
	public function license_field( $options ) {
		$options['fields'][] = array(
			'name'        => $this->item_name,
			'desc'        => __( 'Enter the License Key for this extension', 'sliced-invoices' ),
			'id'          => $this->key_name,
			'type'        => 'text',
	...
	public function after_field( $args, $field ) {
		...
		if ( $this->license_status ) {
			...
			$status = '<span class="license-status license-'. $status .'">' . sprintf( esc_html__( 'License: %s', 'sliced-invoices' ), $status ) . '</span>';
		}

		$nonce = wp_nonce_field( 'sliced_license_nonce', 'sliced_license_nonce_' . $this->option_slug, false, false );

		$id = $this->option_slug . ( 'valid' === $this->license_status ? '_license_deactivate' : '_license_activate' );

		$label = 'valid' === $this->license_status
			? esc_html__( 'Deactivate License', 'sliced-invoices' )
			: esc_html__( 'Activate License', 'sliced-invoices' );
